...
import {responseSuggestError} from '~/services/DataErrorCallApi'
    ...
    useEffect(() => {
        ...
        const fetchAPI = async () => {
            if (!seeAll) {
                ...
                result ? setSuggests(result) : setSuggests(responseSuggestError)
            } else {
    ...
    return (
        <div className={cx('wrapper', { shrink: shrink })}>
            <div className={cx('inner')}>
                ...
                {!context.currentUser && <div className={cx('login')}>
                ...
                {context.currentUser && <div className={cx('following')}>
                    <p className={cx('title')}>Following accounts</p>
                    {responseSuggestError.map((suggest) => {
                        return <SuggestedAccounts key={suggest.id++} data={suggest} following />
                    })}
